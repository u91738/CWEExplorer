...
def expand_training_paths(
    graph:nx.DiGraph, block_len:int, max_path_len:int, max_successors:int,
    ...
    for i in expand_paths(graph, queue, max_path_len, max_successors, filter_edge):
        res.add(disasm_path(graph, i, block_len) + bytes([edge_kind.value]))
...
res = set()
expand_training_paths(graph, args.max_path_len, args.block_len, args.max_successors, res, EdgeKind.GOOD)
expand_training_paths(graph, args.max_path_len, args.block_len, args.max_successors, res, EdgeKind.BAD)
